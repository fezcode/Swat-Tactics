...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  togglePause: () => {
    ...
    else if (['game_over', 'level_complete', 'victory', 'level_intro'].includes(phase)) set({ phase: 'main_menu' });
  },
  ...
  updateTurretFireTime: (id) => set(state => ({ turrets: state.turrets.map(t => t.id === id ? { ...t, lastFireTime: Date.now() } : t) })),
